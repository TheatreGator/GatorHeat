...
if not df_sales.empty:
    ...
    tab_map, tab_data, tab_forecast, tab_optimize = st.tabs(["🗺️ Interactive Heatmap", "💷 Revenue Data", "📈 What-If Forecast", "🧠 AI Price Optimizer"])
    ...
    with tab_map:
        ...
        event = st.plotly_chart(final_fig, width="stretch", on_select="rerun", selection_mode=('box', 'lasso', 'points'))
    ...
    with tab_forecast:
        if has_p and has_r:
            ...
            sold_for_forecast = merged_df[merged_df['Count'] > 0].copy()
            sold_for_forecast['Identifier'] = sold_for_forecast['Area'] + " - " + sold_for_forecast['Combined_Seat']
            ...
            if custom_selected_seats:
                ...
                sold_for_forecast.loc[sold_for_forecast['Identifier'].isin(custom_selected_seats), 'Expected_Price'] = '⭐ Custom Selection'
            
            if not sold_for_forecast.empty:
                agg_dict_forecast = {'Count': 'sum', 'Total_Revenue': 'sum'}
                if has_v: agg_dict_forecast['Total_Value'] = 'sum'
                
                band_stats = sold_for_forecast.groupby('Expected_Price').agg(agg_dict_forecast).reset_index()
                band_stats['Current_Avg_Price'] = (band_stats['Total_Revenue'] / band_stats['Count']).fillna(0)
                
                if has_v:
                    band_stats['Current_Avg_Value'] = (band_stats['Total_Value'] / band_stats['Count']).fillna(0)
                ...
                with col_bands:
                    ...
                    for _, row in band_stats.iterrows():
                        ...
                        if has_v:
                            ...
                            label_str = f"Band '{band}' Price (£) - (Paid avg £{curr_price:.2f} | Original avg £{curr_value:.2f} across {count_val} tickets)"
                        else:
                            label_str = f"Band '{band}' Price (£) - (Currently averaging £{curr_price:.2f} across {count_val} tickets)"
    ...
    with tab_optimize:
        ...
        # Calculate mathematical distance from Center Aisle to break ties
        opt_df['Dist_Center'] = abs(opt_df['X'] - 500)
        
        # DESIRABILITY ALGORITHM: 
        # 1. Highest 'Count' (Most Popular) 
        # 2. Lowest 'Y' (Closest to Stage) 
        # 3. Lowest 'Dist_Center' (Most Central)
        opt_df = opt_df.sort_values(by=['Count', 'Y', 'Dist_Center'], ascending=[False, True, True]).reset_index(drop=True)
        
        # Split seats into mathematically even pricing tiers based on rank
        ...
        for i, col in enumerate(tier_cols):
            ...
            # Calculate what the average price for these specific seats historically WAS to provide a realistic baseline
        ...
        # Generate the Plotly Map for the optimized tiers
        opt_fig = go.Figure()
