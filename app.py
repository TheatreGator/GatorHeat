...
def generate_smart_insights(merged_df, total_capacity, num_performances, uses_revenue=False, uses_bands=False):
    st.markdown("### 🤖 Smart Insights")
    sold_df = merged_df[merged_df['Count'] > 0]
    total_sales = sold_df['Count'].sum()
    
    # Calculate accurate sell-through based on total available inventory across the run
    ...
    if uses_bands:
        ...
        # Inventory per band is the number of seats in that band multiplied by the number of performances
...
if not df_sales.empty:
    ...
    with col_perf:
        num_performances = st.number_input("Number of Performances in Data", min_value=1, value=1, step=1, help="Multiply capacity by this number to get an accurate sell-through rate for runs with multiple shows.")
    ...
    # Display AI Insights
    generate_smart_insights(merged_df, TOTAL_VENUE_CAPACITY, num_performances, uses_revenue=has_revenue, uses_bands=has_pricing)
    st.markdown("---")
