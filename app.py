...
def process_data(filtered_sales, df_layout, area_c, seat_c, count_c, price_c, value_c, band_c):
    ...
    def safe_match(layout_area, data, target_col):
        ...
        return exact if not exact.empty else data[data[target_col].apply(lambda x: str(x) in layout_area or layout_area in str(x))]
    ...
    # --- Price Band Backfilling (Solves the "Unmapped" issue for Forecasts) ---
    if has_p:
        area_modes = {}
        for a in merged_df['Area'].unique():
            bands = merged_df[(merged_df['Area'] == a) & (merged_df['Expected_Price'] != 'Unmapped')]['Expected_Price']
            if not bands.empty:
                area_modes[a] = bands.mode()[0]
                
        merged_df['Expected_Price'] = merged_df.apply(
            lambda r: area_modes.get(r['Area'], 'Unmapped') if r['Expected_Price'] == 'Unmapped' else r['Expected_Price'], axis=1
        )
        
    return merged_df, has_r, has_p, has_v
...
def build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=None):
    ...
    fig.update_layout(
        ...
        hovermode="closest", 
        showlegend=(metric_choice == "Price Band"), 
        dragmode='lasso'
    )
...
if not df_sales.empty:
    ...
    with c8: perf_col = st.selectbox("Event Date/Time (Optional)", ["None"] + list(df_sales.columns), index=get_col_index("Event Date/Time", df_sales.columns, has_none=True))
    with c9: num_performances = st.number_input("Total Performances", min_value=1, value=1, step=1, help="Multiply capacity by this number to get an accurate sell-through rate.")
    
    if perf_col != "None":
        unique_perfs = df_sales[perf_col].dropna().unique().tolist()
        selected_perfs = st.multiselect("Select Performance(s) to Analyze:", unique_perfs, default=unique_perfs)
        df_sales = df_sales[df_sales[perf_col].isin(selected_perfs)]
        num_performances = len(selected_perfs) if selected_perfs else 1
    ...
    tab_map, tab_data, tab_forecast, tab_optimize = st.tabs(["🗺️ Interactive Heatmap", "💷 Revenue Data", "📈 Predictive Forecaster", "🧠 AI Price Optimizer"])
    
    custom_selected_seats = []
    ...
    with tab_map:
        ...
        if date_col != "None" and max_date is not None:
            ...
            with c_anim1: play_btn = st.button("▶️ Play Week-by-Week Animation")
        else:
            play_btn = False
        ...
        final_fig = build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=f"Sales up to: {selected_date.strftime('%Y-%m-%d')}" if date_col != "None" and max_date else None)
        
        with c_anim2 if date_col != "None" else c_anim1:
            raw_html = final_fig.to_html(full_html=True, include_plotlyjs='cdn', default_width="100%", default_height="100vh")
            responsive_html = raw_html.replace("<head>", "<head>\n    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n    <style>body { margin: 0; padding: 0; overflow: hidden; background-color: white; }</style>")
            
            st.download_button(label="🔗 Download Shareable Interactive Map (HTML)", data=responsive_html, file_name="alhambra_interactive_heatmap.html", mime="text/html")
            
        st.markdown("*💡 **Tip:** Use the Lasso or Box Select tools on the map menu to highlight specific seats. They will instantly appear in the Predictive Forecaster tab.*")
        
        event = st.plotly_chart(final_fig, use_container_width=True, on_select="rerun", selection_mode=('box', 'lasso', 'points'))
        
        if event and 'selection' in event and 'points' in event['selection']:
            for pt in event['selection']['points']:
                if 'text' in pt: custom_selected_seats.append(pt['text'])
    ...
    with tab_forecast:
        if has_p and has_r:
            st.subheader("📈 Predictive Forecaster & What-If Modeler")
            st.markdown("Accept your current ticket sales as a locked baseline, predict where the remaining audience will sit based on desirability, and apply custom prices to those forecasted seats.")
            
            total_inventory = TOTAL_VENUE_CAPACITY * num_performances
            total_sold = merged_df['Count'].sum()
            remaining_cap = int(total_inventory - total_sold)
            
            st.markdown("---")
            st.markdown("#### 1. Future Volume Prediction")
            add_sales = st.slider("🔮 Forecast Additional Ticket Sales:", min_value=0, max_value=remaining_cap, value=0, step=1, help="The AI will proportionally allocate these seats to the best remaining physical inventory in the venue based on historical patterns.")
            
            # --- Proportional Allocation Algorithm ---
            pred_df = merged_df.copy()
            pred_df['Identifier'] = pred_df['Area'] + " - " + pred_df['Combined_Seat']
            
            if custom_selected_seats:
                st.info(f"✨ **{len(custom_selected_seats)} Custom Seats Selected!** They have been grouped below so you can forecast them independently.")
                pred_df.loc[pred_df['Identifier'].isin(custom_selected_seats), 'Expected_Price'] = '⭐ Custom Selection'
                
            pred_df['Available'] = num_performances - pred_df['Count']
            pred_df['Available'] = pred_df['Available'].clip(lower=0)
            pred_df['Forecast_Sold'] = 0
            
            if add_sales > 0:
                # Calculate Area Proportions based on actual sales
                area_counts = pred_df.groupby('Area')['Count'].sum()
                total_c = area_counts.sum()
                
                # Fallback to pure capacity if 0 tickets sold
                area_weights = area_counts if total_c > 0 else pred_df.groupby('Area')['Available'].sum()
                area_probs = (area_weights / area_weights.sum()).to_dict()
                
                # Sort seats by physical desirability (Closest to stage, then closest to center aisle)
                pred_df['Dist_Center'] = abs(pred_df['X'] - 500)
                pred_df = pred_df.sort_values(by=['Y', 'Dist_Center'], ascending=[True, True])
                
                rem_sales = add_sales
                
                # Pass 1: Proportional Allocation per Area
                area_allocations = {}
                for area, prob in area_probs.items():
                    alloc = int(add_sales * prob)
                    area_avail = pred_df[pred_df['Area'] == area]['Available'].sum()
                    actual_alloc = min(alloc, area_avail) # Strictly enforce capacity limits
                    area_allocations[area] = actual_alloc
                    rem_sales -= actual_alloc
                    
                for area, alloc in area_allocations.items():
                    if alloc <= 0: continue
                    area_idx = pred_df[(pred_df['Area'] == area) & (pred_df['Available'] > 0)].index
                    for idx in area_idx:
                        if alloc <= 0: break
                        take = min(pred_df.at[idx, 'Available'], alloc)
                        pred_df.at[idx, 'Forecast_Sold'] += take
                        pred_df.at[idx, 'Available'] -= take
                        alloc -= take
                        
                # Pass 2: Spillover (Any remaining sales caused by rounding or capped areas go to absolute best remaining seats)
                if rem_sales > 0:
                    avail_idx = pred_df[pred_df['Available'] > 0].index
                    for idx in avail_idx:
                        if rem_sales <= 0: break
                        take = min(pred_df.at[idx, 'Available'], rem_sales)
                        pred_df.at[idx, 'Forecast_Sold'] += take
                        pred_df.at[idx, 'Available'] -= take
                        rem_sales -= take

            # --- Group Statistics ---
            agg_dict_forecast = {'Count': 'sum', 'Total_Revenue': 'sum', 'Forecast_Sold': 'sum'}
            if has_v: agg_dict_forecast['Total_Value'] = 'sum'
            
            band_stats = pred_df.groupby('Expected_Price').agg(agg_dict_forecast).reset_index()
            band_stats['Current_Avg_Price'] = (band_stats['Total_Revenue'] / band_stats['Count']).fillna(0)
            if has_v: band_stats['Current_Avg_Value'] = (band_stats['Total_Value'] / band_stats['Count']).fillna(0)
            
            st.markdown("---")
            st.markdown("#### 2. Target Price Adjustment")
            st.markdown("Set the target price for the newly forecasted audience below.")
            ...
            with col_bands:
                ...
                for _, row in band_stats.iterrows():
                    ...
                    curr_price = row['Current_Avg_Price']
                    forecasted_vol = int(row['Forecast_Sold'])
                    
                    if has_v:
                        curr_value = row['Current_Avg_Value']
                        label_str = f"Band '{band}' target price for {forecasted_vol} new tickets (Historically paid avg £{curr_price:.2f} | Orig avg £{curr_value:.2f})"
                    else:
                        label_str = f"Band '{band}' target price for {forecasted_vol} new tickets (Historically paid avg £{curr_price:.2f})"
                    
                    new_prices[band] = st.number_input(label_str, value=float(curr_price), step=1.0)
            
            with col_results:
                st.markdown("#### Combined Revenue Results")
                ...
                for _, row in band_stats.iterrows():
                    ...
                    if band == "Unmapped":
                        ...
                        base_revenue += row['Total_Revenue']
                        forecast_revenue += row['Forecast_Sold'] * new_prices[band]
                
                total_proj = base_revenue + forecast_revenue
                
                st.metric("Locked Baseline (Actual Sales)", f"£{base_revenue:,.2f}")
                st.metric("Forecasted Additional Income", f"+ £{forecast_revenue:,.2f}")
                st.metric("Total Projected Revenue", f"£{total_proj:,.2f}", delta=f"£{forecast_revenue:,.2f}")

        else:
            st.warning("⚠️ To use the Forecasting tool, you must map both a **Revenue/Price Paid Column** and a **Price Band Column**.")
            
    with tab_optimize:
        st.subheader("🧠 AI Dynamic Price Optimizer")
        st.markdown("This tool analyzes historical sales volume and physical seat locations to re-draw your venue's pricing map, maximizing yield.")
        
        c_opt1, c_opt2 = st.columns([1, 3])
        with c_opt1:
            num_tiers = st.number_input("Number of Optimized Tiers", min_value=2, max_value=8, value=4)
        
        opt_df = merged_df.copy()
        
        opt_df['Dist_Center'] = abs(opt_df['X'] - 500)
        
        opt_df = opt_df.sort_values(by=['Count', 'Y', 'Dist_Center'], ascending=[False, True, True]).reset_index(drop=True)
        
        tier_labels = [f"Tier {i+1} (Premium)" if i==0 else f"Tier {i+1}" for i in range(num_tiers)]
        opt_df['Suggested_Tier'] = pd.qcut(opt_df.index, q=num_tiers, labels=tier_labels)
        
        st.markdown(f"#### Set Target Prices for the {num_tiers} New Tiers")
        st.markdown(f"The AI has divided your **{len(opt_df)} seats** into {num_tiers} optimized zones based on demand. Set your hypothetical prices below to see the potential revenue uplift.")
        ...
        for i, col in enumerate(tier_cols):
            ...
            tier_seats = opt_df[opt_df['Suggested_Tier'] == tier]
            curr_avg = (tier_seats['Total_Revenue'].sum() / tier_seats['Count'].sum()) if has_r and tier_seats['Count'].sum() > 0 else 0
            
            with col:
                tier_prices[tier] = st.number_input(f"{tier} Price (£)", value=float(curr_avg) if curr_avg > 0 else float(50 - (i*5)), step=1.0)
                if has_r and curr_avg > 0:
                    st.caption(f"Historical Avg Paid: £{curr_avg:.2f}")
                    
        opt_df['Optimized_Revenue'] = opt_df.apply(lambda row: row['Count'] * tier_prices[row['Suggested_Tier']], axis=1)
        
        base_rev = opt_df['Total_Revenue'].sum() if has_r else 0
        new_rev = opt_df['Optimized_Revenue'].sum()
        diff = new_rev - base_rev
        
        st.markdown("---")
        c_res1, c_res2 = st.columns(2)
        with c_res1:
            st.metric("Historical Target Revenue", f"£{base_rev:,.2f}" if has_r else "N/A")
        with c_res2:
            st.metric("Optimized Potential Revenue", f"£{new_rev:,.2f}", delta=f"£{diff:,.2f}" if has_r else None)
            
        st.markdown("#### Optimized Yield Map")
        st.markdown("This map shows exactly where your new price bands should be drawn based on the physical popularity algorithm.")
        
        opt_fig = go.Figure()
        color_seq = px.colors.qualitative.Bold
        
        for i, tier in enumerate(tier_labels):
            t_df = opt_df[opt_df['Suggested_Tier'] == tier]
            opt_fig.add_trace(go.Scatter(
                x=t_df['X'], y=t_df['Y'], mode='markers',
                marker=dict(size=14, color=color_seq[i % len(color_seq)], line=dict(width=1, color='DarkSlateGrey')),
                name=tier, text=t_df['Area'] + " - " + t_df['Combined_Seat'],
                customdata=t_df[['Suggested_Tier', 'Count']],
                hovertemplate="<b>%{text}</b><br>Suggested: %{customdata[0]}<br>Historical Sales: %{customdata[1]}<extra></extra>"
            ))
        
        opt_fig.add_annotation(x=500, y=70, text="<b>STALLS</b>", showarrow=False, font=dict(size=18, color="black"))
        opt_fig.add_annotation(x=500, y=stalls_bottom + 80, text="<b>DRESS CIRCLE</b>", showarrow=False, font=dict(size=18, color="black"))
        opt_fig.add_annotation(x=500, y=dress_bottom + 80, text="<b>UPPER CIRCLE</b>", showarrow=False, font=dict(size=18, color="black"))
        
        opt_fig.update_layout(
            plot_bgcolor='white', width=1200, height=1100, 
            yaxis=dict(autorange='reversed', showgrid=False, zeroline=False, visible=False), 
            xaxis=dict(showgrid=False, zeroline=False, visible=False), 
