...
def generate_smart_insights(merged_df, total_capacity, uses_revenue=False, uses_bands=False):
    ...
    summary_text = f"**Capacity:** {int(total_sales)} tickets sold across {len(sold_df)} unique seats. This is a **{sell_through:.1f}% sell-through rate**.\n\n"
    summary_text += f"**Volume Trends:** The highest volume section is the **{busiest_area}**. Top seat is **{top_seat['Area']} - {top_seat['Combined_Seat']}** ({int(top_seat['Count'])} sold)."
    ...
    if uses_bands:
        ...
        band_capacity = merged_df.groupby('Expected_Price')['Count'].count()
        
        st.markdown(summary_text)
        st.markdown("**Pricing Strategy Performance:**")
        
        # Calculate sell-through per price band
        band_metrics = pd.DataFrame({
            'Total Available Seats': band_capacity,
            'Unique Seats Sold': sold_df.groupby('Expected_Price')['Count'].count(),
            'Total Ticket Volume': band_sales
        }).fillna(0)
        
        band_metrics['Sell-Through %'] = (band_metrics['Unique Seats Sold'] / band_metrics['Total Available Seats']) * 100
...
if not df_sales.empty:
    ...
    with c5: band_col = st.selectbox("Price Band Column (Optional)", ["None"] + list(df_sales.columns), index=0)
    ...
    if band_col != "None":
        agg_dict[band_col] = 'first' # Assuming a seat belongs to a static band per run
        has_pricing = True
    ...
    for _, layout_row in df_layout.iterrows():
        ...
        merged_records.append({
            ...
            'Count': count, 'Total_Revenue': revenue, 'Avg_Price': avg_price, 'Expected_Price': expected_price
        })
        
    merged_df = pd.DataFrame(merged_records)
    
    # Display AI Insights
    generate_smart_insights(merged_df, TOTAL_VENUE_CAPACITY, uses_revenue=has_revenue, uses_bands=has_pricing)
    st.markdown("---")
    ...
    with tab_map:
        ...
        if metric_choice == "Price Band":
            # Discrete Categorical Plotting for Pricing
            ...
            if not sold_df.empty:
                hover_template = "<b>%{text}</b><br>Times Sold: %{customdata[0]}<br>Total Revenue: £%{customdata[1]:.2f}<br>Avg Price/Seat: £%{customdata[2]:.2f}"
                if has_pricing: hover_template += "<br>Price Band: %{customdata[3]}"
                hover_template += "<extra></extra>"

                custom_data = sold_df[['Count', 'Total_Revenue', 'Avg_Price', 'Expected_Price']] if has_pricing else sold_df[['Count', 'Total_Revenue', 'Avg_Price']]
    ...
    with tab_data:
        ...
        display_cols = ['Area', 'Combined_Seat', 'Count', 'Total_Revenue', 'Avg_Price']
        if has_pricing: display_cols.append('Expected_Price')
            
        display_df = merged_df[display_cols].copy()
        rename_map = {'Combined_Seat': 'Seat', 'Total_Revenue': 'Total Revenue (£)', 'Avg_Price': 'Avg Price (£)'}
