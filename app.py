...
def build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=None):
    ...
    # REVERTED TO FIXED DIMENSIONS FOR CLEAN DESKTOP VIEW
    fig.update_layout(
        plot_bgcolor='white', 
        width=1200, 
        height=1100, 
...
def generate_smart_insights(merged_df, total_capacity, num_performances, has_r, has_p, has_v):
    ...
    if has_p:
        ...
        st.dataframe(band_metrics.sort_index(ascending=True), width="stretch", column_config={"Sell-Through %": st.column_config.NumberColumn(format="%.1f%%")})
    else:
...
if not df_sales.empty:
    ...
    with tab_map:
        ...
        st.markdown("*💡 **Tip:** Use the Lasso or Box Select tools on the map menu to highlight specific seats. They will instantly appear in the What-If Forecast tab.*")
    ...
    with tab_forecast:
        if has_p and has_r:
            ...
            add_sales = st.slider("🔮 Forecast Additional Ticket Sales:", min_value=0, max_value=remaining_cap, value=0, step=1, help="The AI will allocate these seats to the best remaining physical inventory in the venue based on historical popularity.")
            ...
            # Rank unsold seats by Area Popularity, Closeness to Stage, and Centrality
            area_pop = pred_df.groupby('Area')['Count'].sum().reset_index().rename(columns={'Count': 'Area_Pop'})
            pred_df = pred_df.merge(area_pop, on='Area')
            pred_df['Dist_Center'] = abs(pred_df['X'] - 500)
            pred_df = pred_df.sort_values(by=['Area_Pop', 'Y', 'Dist_Center'], ascending=[False, True, True]).reset_index(drop=True)
            
            # Allocate the slider volume into empty seats
            pred_df['Forecast_Sold'] = 0
            rem = add_sales
            for idx, row in pred_df.iterrows():
                if rem <= 0: break
                take = min(row['Available'], rem)
            ...
            st.markdown("---")
            st.markdown("#### 2. Price Adjust (Apply to Forecasted Audience)")
