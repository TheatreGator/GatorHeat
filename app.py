...
if not df_sales.empty:
    ...
    with c4: price_col = st.selectbox("Revenue/Price Paid Column (Optional)", ["None"] + list(df_sales.columns), index=0)
    with c5: value_col = st.selectbox("Total Value Column (Optional)", ["None"] + list(df_sales.columns), index=0)
    with c6: band_col = st.selectbox("Price Band Column (Optional)", ["None"] + list(df_sales.columns), index=0)
    
    # 2. Performance Settings
    st.markdown("#### Performance Settings")
    col_perf, _ = st.columns([1, 4])
    with col_perf:
        num_performances = st.number_input("Number of Performances in Data", min_value=1, value=1, step=1, help="Multiply capacity by this number to get an accurate sell-through rate.")
    ...
    with tab_map:
        ...
        if metric_choice == "Price Band":
            ...
            if not sold_df.empty:
                # Dynamically build hover template based on selected columns
    ...
    with tab_data:
        ...
        # Determine the column to sort by natively
