...
with col1:
    sales_file = st.file_uploader("Upload Sales Data", type=["csv", "xlsx", "xls"])
with col2:
...
def generate_smart_insights(merged_df, total_capacity, num_performances, uses_revenue=False, uses_bands=False, uses_value=False):
    ...
    if sold_df.empty:
        st.info("Upload sales data to generate insights. If you are using the date slider, ensure you have selected a date with active sales.")
        return
    ...
    if uses_bands:
        ...
        band_metrics = pd.DataFrame({
            'Total Available Tickets': band_capacity,
            'Tickets Sold': band_sales
        }).fillna(0)
        
        band_metrics['Sell-Through %'] = (band_metrics['Tickets Sold'] / band_metrics['Total Available Tickets']) * 100
        
        st.dataframe(
            band_metrics.sort_index(ascending=True), 
            use_container_width=True,
            column_config={"Sell-Through %": st.column_config.NumberColumn(format="%.1f%%")}
        )
    else:
...
if not df_sales.empty:
    ...
    with c4: price_col = st.selectbox("Revenue/Price Paid (Optional)", ["None"] + list(df_sales.columns), index=0)
    with c5: value_col = st.selectbox("Total Value (Optional)", ["None"] + list(df_sales.columns), index=0)
    with c6: band_col = st.selectbox("Price Band (Optional)", ["None"] + list(df_sales.columns), index=0)
    
    c7, c8, _ = st.columns(3)
    with c7: date_col = st.selectbox("Date of Purchase (Optional)", ["None"] + list(df_sales.columns), index=0)
    ...
    if date_col != "None":
        ...
        if not valid_dates.empty:
            ...
            selected_date = st.slider(
                "Drag to view the venue fill up over time:", 
                min_value=min_date, 
                max_value=max_date, 
                value=max_date,
                format="YYYY-MM-DD"
            )
            # Filter the dataset before aggregating
            df_sales = df_sales[df_sales[date_col].dt.date <= selected_date]
            st.markdown("---")
    ...
    if band_col != "None":
        agg_dict[band_col] = 'first' 
        has_pricing = True

    df_grouped = df_sales.groupby([area_col, seat_col]).agg(agg_dict).reset_index()
    
    # Standardize column names for processing
    df_grouped.rename(columns={count_col: 'Count'}, inplace=True)
    if has_revenue:
        df_grouped.rename(columns={price_col: 'Total_Revenue'}, inplace=True)
        df_grouped['Avg_Price'] = (df_grouped['Total_Revenue'] / df_grouped['Count']).fillna(0)
    if has_value:
        df_grouped.rename(columns={value_col: 'Total_Value'}, inplace=True)
    if has_pricing:
        df_grouped.rename(columns={band_col: 'Expected_Price'}, inplace=True)

    def safe_match(layout_area, data, target_col):
        exact = data[data[target_col] == layout_area]
        if not exact.empty: return exact
        return data[data[target_col].apply(lambda x: x in layout_area or layout_area in x)]
    ...
    for _, layout_row in df_layout.iterrows():
        ...
        area_match = safe_match(area, df_grouped, area_col)
        seat_match = area_match[area_match[seat_col] == seat]
        
        count = seat_match['Count'].sum() if not seat_match.empty else 0
        revenue = seat_match['Total_Revenue'].sum() if has_revenue and not seat_match.empty else 0
        value = seat_match['Total_Value'].sum() if has_value and not seat_match.empty else 0
        avg_price = seat_match['Avg_Price'].mean() if has_revenue and not seat_match.empty and count > 0 else 0
        
        reduction_pct = ((value - revenue) / value * 100) if has_revenue and has_value and value > 0 else 0
        
        expected_price = "Unmapped"
        if has_pricing and not seat_match.empty:
            expected_price = seat_match['Expected_Price'].values[0]
    ...
    # Display AI Insights
    generate_smart_insights(merged_df, TOTAL_VENUE_CAPACITY, num_performances, uses_revenue=has_revenue, uses_bands=has_pricing, uses_value=has_value)
    st.markdown("---")
    ...
    with tab_map:
        ...
        with c_right:
            ...
            if has_revenue: metric_options.append("Total Revenue")
            if has_pricing: metric_options.append("Price Band")
        ...
        if metric_choice == "Price Band":
            unique_bands = map_df['Expected_Price'].unique()
            color_sequence = px.colors.qualitative.Pastel + px.colors.qualitative.Set3
            
            for i, band in enumerate(unique_bands):
                band_df = map_df[map_df['Expected_Price'] == band]
                marker_color = '#e0e0e0' if str(band) == "Unmapped" else color_sequence[i % len(color_sequence)]
                    
                fig.add_trace(go.Scatter(
                    x=band_df['X'], y=band_df['Y'], mode='markers',
                    marker=dict(size=marker_size, color=marker_color, line=dict(width=1, color='DarkSlateGrey')),
                    name=str(band),
                    text=band_df['Area'] + " - " + band_df['Combined_Seat'],
                    customdata=band_df[['Expected_Price', 'Count']],
                    hovertemplate="<b>%{text}</b><br>Band: %{customdata[0]}<br>Times Sold: %{customdata[1]}<extra></extra>"
                ))
            ...
            if not sold_df.empty:
                hover_template = "<b>%{text}</b><br>Times Sold: %{customdata[0]}"
                custom_cols = ['Count']
                
                if has_revenue:
                    hover_template += "<br>Total Revenue: £%{customdata[1]:.2f}<br>Avg Price/Seat: £%{customdata[2]:.2f}"
                    custom_cols.extend(['Total_Revenue', 'Avg_Price'])
                    
                    if has_value:
                        hover_template += "<br>Total Value: £%{customdata[3]:.2f}<br>Reduction: %{customdata[4]:.1f}%"
                        custom_cols.extend(['Total_Value', 'Reduction_Pct'])
                
                if has_pricing:
                    custom_cols.append('Expected_Price')
                    idx = len(custom_cols) - 1
                    hover_template += f"<br>Price Band: %{{customdata[{idx}]}}"
                    
                hover_template += "<extra></extra>"

                custom_data = sold_df[custom_cols]
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name="Sold", text=sold_df['Area'] + " - " + sold_df['Combined_Seat'], customdata=custom_data, hovertemplate=hover_template
                ))
    ...
    with tab_data:
        ...
        if has_revenue:
            display_cols.extend(['Total_Revenue', 'Avg_Price'])
        if has_revenue and has_value:
            display_cols.extend(['Total_Value', 'Reduction_Pct'])
        if has_pricing: 
            display_cols.append('Expected_Price')
        ...
        if has_pricing: rename_map['Expected_Price'] = 'Price Band'
        display_df.rename(columns=rename_map, inplace=True)
        
        sort_col = 'Total Revenue (£)' if has_revenue else 'Count'
            ...
            if not sold_for_forecast.empty:
                ...
                with col_bands:
                    ...
                    for _, row in band_stats.iterrows():
                        ...
                        ticket_volume = row['Count']
                        new_prices[band] = st.number_input(f"Band '{band}' Price (£) - (Currently averaging £{curr_price:.2f} across {int(ticket_volume)} tickets)", value=float(curr_price), step=1.0)
                ...
                with col_results:
                    ...
                    diff = forecast_revenue - base_revenue
                    
                    st.metric("Current Base Revenue", f"£{base_revenue:,.2f}")
                    st.metric("Forecasted Revenue", f"£{forecast_revenue:,.2f}", delta=f"£{diff:,.2f}")
            else:
