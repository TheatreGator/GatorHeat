...
def process_data(filtered_sales, df_layout, area_c, seat_c, count_c, price_c, value_c, band_c):
    ...
    def safe_match(layout_area, data, target_col):
        # BUG FIX: Prevents a KeyError when the mapped data goes completely empty!
        if data.empty:
            return data
...
def build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=None):
    ...
    fig.update_layout(plot_bgcolor='white', width=1200, height=1100, yaxis=dict(autorange='reversed', showgrid=False, zeroline=False, visible=False), xaxis=dict(showgrid=False, zeroline=False, visible=False), hovermode="closest", showlegend=(metric_choice == "Price Band"), dragmode='lasso')
...
if not df_sales.empty:
    ...
    with tab_map:
        ...
        with c_anim2 if date_col != "None" else c_anim1:
            html_string = final_fig.to_html(full_html=True, include_plotlyjs='cdn')
            st.download_button(label="🔗 Download Shareable Interactive Map (HTML)", data=html_string, file_name="alhambra_interactive_heatmap.html", mime="text/html")
        ...
        # Bug Fix: Ensuring the valid 'points' keyword is used instead of 'select'
        ...
        if event and 'selection' in event and 'points' in event['selection']:
            for pt in event['selection']['points']:
                if 'text' in pt: # Contains 'Area - Seat' format
                    custom_selected_seats.append(pt['text'])
    ...
    with tab_forecast:
        if has_p and has_r:
            ...
            # Apply custom grouping if seats are selected on the visual map
