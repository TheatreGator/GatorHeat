...
# Helper to merge sales into layout based on selected time-frame
...
# Helper to build the plotly figure map
def build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=None):
    ...
    if title_override:
        fig.update_layout(title=title_override, title_x=0.5, title_font=dict(size=24))
...
if not df_sales.empty:
    ...
    with c7: date_col = st.selectbox("Date of Purchase (Optional for Animation)", ["None"] + list(df_sales.columns), index=0)
    ...
    # Placeholder for AI Insights to render at top
    ...
    with tab_map:
        # Time Slider directly above the map
        ...
        if date_col != "None" and max_date is not None:
            ...
            with c_anim2: gif_btn = st.button("💾 Generate Animation GIF")
        else:
            play_btn, gif_btn = False, False
        ...
        # Build Filtered Data for Slider (Default State)
        df_filtered = df_sales[df_sales[date_col].dt.date <= selected_date] if date_col != "None" else df_sales
        merged_df, has_r, has_p, has_v = process_data(df_filtered, df_layout, area_col, seat_col, count_col, price_col, value_col, band_col)
        
        # Logic for Live Browser Animation
        if play_btn and min_date and max_date:
            curr_d = min_date
            while curr_d <= max_date:
                t_df = df_sales[df_sales[date_col].dt.date <= curr_d]
                ...
                time.sleep(0.5) # Wait half a second before next frame
                curr_d += datetime.timedelta(days=7)
            # Finish on final date
        ...
        # Logic for GIF Export
        elif gif_btn and min_date and max_date:
            try:
                from PIL import Image
                import io
                
                with st.spinner("Rendering GIF frames... this may take a moment."):
                    frames = []
                    curr_d = min_date
                    while curr_d <= max_date + datetime.timedelta(days=7):
                        cutoff_d = min(curr_d, max_date)
                        t_df = df_sales[df_sales[date_col].dt.date <= cutoff_d]
                        t_merged, _, _, _ = process_data(t_df, df_layout, area_col, seat_col, count_col, price_col, value_col, band_col)
                        t_fig = build_figure(t_merged, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v, title_override=f"Sales up to: {cutoff_d.strftime('%Y-%m-%d')}")
                        
                        img_bytes = t_fig.to_image(format="png", width=1200, height=1000)
                        frames.append(Image.open(io.BytesIO(img_bytes)))
                        if curr_d >= max_date: break
                        curr_d += datetime.timedelta(days=7)
                    
                    gif_io = io.BytesIO()
                    frames[0].save(gif_io, format='GIF', save_all=True, append_images=frames[1:], duration=600, loop=0)
                    
                    st.success("GIF Generated Successfully!")
                    st.download_button(label="⬇️ Download GIF", data=gif_io.getvalue(), file_name="sales_animation.gif", mime="image/gif")
                    
                    map_placeholder.plotly_chart(build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v), use_container_width=True)
                    
            except ValueError:
                st.error("⚠️ **Missing Dependencies:** To generate GIFs, you need to install the Plotly image exporter. Open your terminal and run: `pip install -U kaleido pillow`")
                map_placeholder.plotly_chart(build_figure(merged_df, view_filter, metric_choice, marker_size, stalls_bottom, dress_bottom, has_r, has_p, has_v), use_container_width=True)
            except Exception as e:
    ...
    # Populate AI Insights at the top
